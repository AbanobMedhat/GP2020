...
err:
    ...
    return 0;//ble_gap_terminate(peer->conn_handle, BLE_ERR_REM_USER_CONN_TERM);
}
...
blecent_connect_if_interesting(const struct ble_gap_disc_desc *disc)
{
    ...
    rc = ble_gap_connect(own_addr_type, &disc->addr, 100000, NULL,
                         blecent_gap_event, NULL);
    if (rc != 0) {
...
blecent_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_CONNECT:
        ...
        if (event->connect.status == 0) {
			...
			if (rc != 0) {
				ESP_LOGE(tag, "Error while initializing security; rc= %d", rc);
				return 0;
			}
